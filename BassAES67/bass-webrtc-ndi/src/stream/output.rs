...
impl WebRtcOutputStream {
    ...
    pub fn start(&mut self) -> Result<(), String> {
        ...
        let handle = thread::Builder::new()
            .name("webrtc-ndi-output-tx".to_string())
            .spawn(move || {
                #[cfg(windows)]
                unsafe {
                ...
                println!("[WebRtcOutputStream] TX thread started");

                while running.load(Ordering::SeqCst) {
                    frame_count += 1;
                    if frame_count % 250 == 0 {
                        println!("[WebRtcOutputStream] Sent {} frames ({} sec)", frame_count, frame_count / 50);
                    }
