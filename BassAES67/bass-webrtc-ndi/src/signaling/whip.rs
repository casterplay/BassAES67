//! WHIP (WebRTC-HTTP Ingestion Protocol) signaling - RFC 9725.
//!
//! Provides HTTP endpoints for WebRTC signaling:
//! - POST /whip - Receive SDP offer, return SDP answer
//! - PATCH /whip/{id} - Trickle ICE candidate
//! - DELETE /whip/{id} - Close connection
...
impl Default for WhipConfig {
    fn default() -> Self {
        Self {
            ...
            path_prefix: "/whip".to_string(),
        }
    }
}

/// WHIP endpoint handler
...
impl WhipEndpoint {
    ...
    pub fn new(config: WhipConfig, peer_manager: Arc<Mutex<PeerManager>>) -> Self {
        Self {
            config,
            peer_manager,
            shutdown_tx: None,
                            ...
                            Ok((stream, _)) => {
                                ...
                                tokio::spawn(async move {
                                    ...
                                    if let Err(e) = http1::Builder::new()
                                        ...
                                        .await
                                    {
                                        eprintln!("WHIP connection error: {}", e);
                                    }
                                });
                            }
                            Err(e) => {
                                eprintln!("WHIP accept error: {}", e);
                            }
                        }
                    }
                    _ = &mut shutdown_rx => {
...
async fn handle_request(
    ...
    match (method, sub_path) {
        // POST /whip - New connection with SDP offer
        (&Method::POST, "" | "/") => {
...
/// Handle POST request - new connection with SDP offer
async fn handle_post_offer(
    ...
    let result = {
        ...
        // Need to use tokio runtime for async operation
...
async fn handle_patch_ice(
    ...
    // Read body (ICE candidate in SDP format or JSON)
    let body_bytes = req.collect().await?.to_bytes();
    ...
    // Parse ICE candidate (simple format: just the candidate line)
    // Format: "candidate:..." or full SDP fragment
