...
//!   --codec CODEC     Codec to use: pcm, opus, mp2 (default: pcm)
...
use bass_srt::codec::{opus, twolame};
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!("  --codec CODEC     Codec: pcm, opus, mp2 (default: pcm)");
...
fn main() {
    ...
    unsafe {
        ...
        if let Some(ref target) = config.connect_to {
            ...
            // LISTENER MODE - wait for incoming connection
            let addr = SockaddrIn {
                sin_family: 2,
