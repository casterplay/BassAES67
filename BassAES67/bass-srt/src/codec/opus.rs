//! OPUS codec bindings for bass_srt.
//!
...
impl Encoder {
    ...
    pub fn new(
        ...
        let valid_frame_sizes = [
            ...
            format.sample_rate / 25,   // 40ms (integer division rounds down, but this is approximate)
    ...
    /// Create an encoder for audio (music/mixed content) at 48kHz stereo with 5ms frames
...
impl Decoder {
    ...
    /// Create a decoder for 48kHz stereo with 5ms frames
    pub fn new_48k_stereo_5ms() -> Result<Self, CodecError> {
        Self::new(AudioFormat::standard(), 5.0)
    }
...
mod tests {
    ...
    fn test_encoder_create() {
        let encoder = Encoder::new_audio_48k_stereo_5ms();
        assert!(encoder.is_ok());

        let encoder = encoder.unwrap();
        assert_eq!(encoder.frame_size(), 240); // 5ms at 48kHz
        assert_eq!(encoder.total_samples_per_frame(), 480); // 240 * 2 channels
    }

    #[test]
    fn test_decoder_create() {
        let decoder = Decoder::new_48k_stereo_5ms();
        assert!(decoder.is_ok());
    }

    #[test]
    fn test_encode_decode_roundtrip() {
        let mut encoder = Encoder::new_audio_48k_stereo_5ms().unwrap();
        let mut decoder = Decoder::new_48k_stereo_5ms().unwrap();
        ...
        let decoded_samples = decoder.decode(&encoded[..encoded_len], &mut pcm_out, false).unwrap();
        assert_eq!(decoded_samples, encoder.frame_size());
    }
}
