//! mpg123 MPEG audio decoder bindings for bass-rtp.
//!
    ...
    pub fn read(&mut self, output: &mut [u8]) -> Result<usize, CodecError> {
        unsafe {
            ...
            match result {
                ...
                MPG123_NEW_FORMAT => {
                    self.update_format()?;
                    // After format change, try to read again - there may be data available
                    if done == 0 {
                        let mut more_done: usize = 0;
                        let result2 = mpg123_read(
                            self.handle,
                        ...
                        if result2 == MPG123_OK || result2 == MPG123_NEED_MORE || result2 == MPG123_DONE {
                            Ok(more_done)
                        } else {
                            Ok(0)
                        }
