...
//! frequency adjustment based on Livewire clock servo output.
...
use crate::client::{get_lw_stats, is_lw_running};
...
fn calculate_adjusted_interval(base_interval_ms: u32, pll_enabled: bool) -> u64 {
    ...
    // Only apply PLL when Livewire clock is running and locked
    if !is_lw_running() {
        return base_100ns;
    }

    let stats = match get_lw_stats() {
        Some(s) => s,
